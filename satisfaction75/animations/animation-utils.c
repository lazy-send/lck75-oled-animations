...
// static void oled_write_over_anim(const char* text, char *animation) {
//     int len = sizeof(text);
//     int count = 0;

//     for(int i = 0; i < len; i++) {
//         // if ()
//         // animation[i*8 + y] = text[i];
//         count ++;
//     } 
//     oled_write_raw_P(animation, ANIM_SIZE);
// }

static void render_animation(char **tap_frames, char **idle_frames, uint8_t tap_frames_len, uint8_t idle_frames_len) {
